...
func main() {
	...
	fmt.Println(len(inputItems))
	fmt.Println(inputItems[0])
	for _, tc := range inputItems {
		elems := strings.Split(tc, " ")
		count := 0

		for _, ch := range elems[2] {
			if string(ch) == strings.TrimSuffix(elems[1], `:`) {
				count++
			}
		}
		vc := strings.Split(elems[0], "-")
		min, err := strconv.Atoi(vc[0])
		if err != nil {
			panic(err)
		}
		max, err := strconv.Atoi(vc[1])
		if err != nil {
			panic(err)
		}
